...
#SBATCH --nodelist=gpu-hm-001    ### which GPU to use
#SBATCH -t 0-40:00:00        ### The time limit in D-hh:mm:ss format
#SBATCH -o ./out/out_%j.log        ### Where to store the console output (%j is the job number)
#SBATCH -e ./error/error_%j.log      ### Where to store the error output
...
python /trinity/home/jyu/DeepSurvival/batch_scripts/MRI_Only/Train_MRI_Only_TFS_FS.py -n ${DATA} -i ${SPLIT}
