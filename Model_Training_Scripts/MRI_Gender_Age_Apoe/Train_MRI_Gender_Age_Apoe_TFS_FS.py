...
#        img_data = img_data*mask+(mask-1.0)
    ...
    return np.array(img_data), np.array(int(input2)), np.array(input3),np.array(int(input4)), label, time
...
def generate_batch(patients, img_size, img_scale=1.0, mask=None, augment=False, mode=[]):
    ...
    for patient in patients:
        try:
            x, x2, x3, x4, y, t = retrieve_data(patient, img_size, img_scale, mask, augment, mode)
            img_data.append(x)
            sex.append(x2)
            age.append(x3)
            apoe.append(x4)
    ...
    return ([img_data, sex_data, age_data, apoe_data], [label_data_out])
...
def pretrained_model(input_shape):
    ...
    y = concatenate([x1, input2, input3, input4])   # other modes: multiply, concatenate, dot
    y = Dense(32, activation='relu')(y)
    y = Dropout(0.2)(y)
    y = Dense(4, activation='relu')(y)
    final = Dense(1, activation='linear')(y)

    model = Model(inputs=[input1, input2, input3, input4], outputs=final)
...
def cnn_model(input_shape):
   ...
   """
    ...
    y = concatenate([x1, input2, input3,input4])   
...
model.summary()
model_file = 'model_MRI_Gender_Age_Apoe_FS.json'
...
version = 'MRI_Gender_Age_Apoe_TFS_FS_'+args.i
