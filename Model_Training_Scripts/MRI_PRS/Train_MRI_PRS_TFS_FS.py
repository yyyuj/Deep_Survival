...
WANG_DIR = '/trinity/home/jyu/DeepSurvival/Notebooks/BrainAge/'
...
def retrieve_data(patient_index, img_size, img_scale=1.0, mask=None, augment=False, mode=[]):
    ...
    # Get third input (ApoE-e4 carriership)
    input4 = patient_info.get('ApoE4')
    
     # Get Fourth input (PRS)
    input5 =patient_info.get('PRS')
    ...
    if ergoid > 0:
        ...
        try:
            img = nib.load(IMAGE_DIR2+patient_filename)
        except:
...
#        img_data = img_data*mask+(mask-1.0)
    ...
    return np.array(img_data), np.array(int(input2)), np.array(input3),np.array(int(input4)),np.array(input5), label, time
...
def generate_batch(patients, img_size, img_scale=1.0, mask=None, augment=False, mode=[]):
    ...
    for patient in patients:
        try:
            x, x2, x3, x4, x5, y, t = retrieve_data(patient, img_size, img_scale, mask, augment, mode)
            img_data.append(x)
            sex.append(x2)
            age.append(x3)
            apoe.append(x4)
    ...
    return ([img_data, sex_data, age_data, apoe_data, prs_data], [label_data_out])
...
def pretrained_model(input_shape):
    """
    Fine-tuning on MRI_Only_TFS_FS model
    """
    model = tf.keras.models.load_model(MODEL_DIR+'model_MRI_Only_TFS_FS_'+str(args.i)+'.h5', custom_objects={'CoxPH_loss': CoxPH_loss})
    ...
    input4 = Input((1,)) # ApoE-e4 input
    input5 = Input((1,)) #prs input
    x1 = Dense(4, activation='relu')(x)

    #merging braches into final model ----------
    y = concatenate([x1, input2, input3, input4, input5])   # other modes: multiply, concatenate, dot
    y = Dense(32, activation='relu')(y)
    y = Dropout(0.2)(y)
    y = Dense(4, activation='relu')(y)
    final = Dense(1, activation='linear')(y)

    model = Model(inputs=[input1, input2, input3, input4, input5], outputs=final)
...
def cnn_model(input_shape):
   ...
   """
    ...
    y = concatenate([x1, input2, input3,input4,input5])   
    y = Dense(32, activation='relu')(y)
    y = Dropout(0.2)(y)
    y = Dense(4, activation='relu')(y)
    final = Dense(1, activation='linear')(y)
    ...
    #adam_opt = keras.optimizers.Adam(lr=0.0005, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.0001)
    adam_opt = keras.optimizers.Adam(lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.01)
    model.compile(loss=CoxPH_loss, optimizer=adam_opt)
    return model
...
model.summary()
model_file = 'model_MRI_PRS_FS.json'
...
version = 'MRI_PRS_TFS_FS_'+args.i
