...
#        img_data = img_data*mask+(mask-1.0)
    ...
    return np.array(img_data), np.array(int(input2)),np.array(input3),np.array(int(input4)), genomics, label, time
...
def pretrained_model(input_shape):
    ...
    input5 = Input((76,))
    gen1 = Dense(4, activation='relu')(input5)
    x1 = Dense(4, activation='relu')(x)

    #merging braches into final model ----------
    y = concatenate([x1, input2, input3, input4, gen1])   # other modes: multiply, concatenate, dot
...
model_file = 'model_MRI_SNP_FS.json'
...
version = 'MRI_SNP_TFS_FS_'+args.i
